...
class GGNN_Baseline(nn.Module):
    def __init__(self, convnet, role_emb, verb_emb, ggnn, classifier, encoder, query_composer, G1):
    ...
    def forward(self, v_org, gt_verb):
        ...
        out = torch.cat([feature1, out], dim=-1) # [batch_size*6, 1024+300+300+512+512]
        ...
        return role_label_pred, adj_r2r, out

    def calculate_loss(self, gt_verbs, role_label_pred, gt_labels, adj_r2r, gt_adj_r2r):
        ...
        return loss.mean() + adj_r2r_loss.mean()
...
def build_ggnn_baseline(n_roles, n_verbs, num_ans_classes, encoder):
    ...
    classifier = nn.Sequential(
        nn.Dropout(0.5),
        nn.Linear(hidden_size+word_embedding_size*2+512, num_ans_classes)
    )

    return GGNN_Baseline(covnet, role_emb, verb_emb, ggnn, classifier, encoder, query_composer, G1)