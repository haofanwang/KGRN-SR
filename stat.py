...
pickle.dump(prob_v2r_adj_matrix, open('prob_v2r_adj_matrix.pkl', 'wb'))
...
for i, (_, img, verb, labels) in enumerate(train_loader):
    ...
    labels = labels.view(batch_size, -1) # batch_size, 3*6

    gt_adj_a = torch.zeros(batch_size, labels.size(1), labels.size(1)) # batch_size, 18, 18
    gt_adj_r = torch.zeros(batch_size, labels.size(1), labels.size(1)) # batch_size, 18, 18

    for b in range(batch_size):
        temp = prob_v2r_adj_matrix[labels[b]-1, :]
        temp = torch.transpose(temp,0,1)[labels[b]-1, :]
        gt_adj_a[b] = temp.transpose(0,1)
    for b in range(batch_size):
        temp = prob_r2r_adj_matrix[labels[b]-1, :]
        temp = torch.transpose(temp, 0,1)[labels[b]-1, :]
