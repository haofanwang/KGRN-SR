...
from models import ggnn_r2v

import logging

logging.basicConfig(filename ='./logs/main_ggnn_r2v.txt', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=1000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, labels) in enumerate(train_loader):
            ...
            role_predict, adj_r2v = pmodel(img, verb)
            loss = model.calculate_loss(verb, role_predict, labels, adj_r2v, gt_adj_r2v)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, labels) in enumerate(dev_loader):
            ...
            role_predict, adj_r2v = model(img, verb)
...
def main():
    ...
    parser.add_argument('--model_saving_name', default="main_ggnn_r2v", type=str, help='saving name of the outpul model')
    ...
    model = getattr(ggnn_r2v, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder)
