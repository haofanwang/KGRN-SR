...
from models import ggnn_r2r_r2v_w

import logging

logging.basicConfig(filename ='./logs/main_ggnn_r2r_r2v_w_tda.txt', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, model_name, model_saving_name, eval_frequency=1000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, labels) in enumerate(train_loader):
            ...
            #     temp = prob_r2v_adj_matrix[temp_labels[b]-1, :]
            #     temp = torch.transpose(temp,0,1)[temp_labels[b]-1, :]
            #     gt_adj_r2v[b] = temp.transpose(0,1)
            # for b in range(batch_size):
            #     temp = prob_r2r_adj_matrix[temp_labels[b]-1, :]
            #     temp = torch.transpose(temp, 0,1)[temp_labels[b]-1, :]
            #     gt_adj_r2r[b] = temp.transpose(0,1)  
            ...
            role_predict, adj_r2r, adj_r2v = pmodel(img, verb)
            loss = model.calculate_loss(verb, role_predict, labels, adj_r2r, adj_r2v, gt_adj_r2r, gt_adj_r2v)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, labels) in enumerate(dev_loader):
            ...
            role_predict, adj_r2r, adj_r2v = model(img, verb)
...
def main():
    ...
    parser.add_argument('--model_saving_name', default="main_ggnn_r2r_r2v_w_tda", type=str, help='saving name of the outpul model')
    ...
    model = getattr(ggnn_r2r_r2v_w, constructor)(encoder.get_num_roles(),encoder.get_num_verbs(), encoder.get_num_labels(), encoder)
